...
def _parse_args():
    ...
    参数说明：
        ...
        sample_fps 默认值为 16
        要生成 15 秒的视频，您需要的帧数计算如下：
        所需帧数 = 视频时长(秒) × 帧率 = 15 × 16 = 240 帧
        但是，根据代码要求，frame_num 必须是 4n+1 的形式（即除以4余1的数）。让我们检查一下：
        240 ÷ 4 = 60 余 0，不符合要求
        最接近且符合要求的数是 241（60×4+1）
        因此，要生成约 15 秒的视频，您应该将 --frame_num 设置为 241：
